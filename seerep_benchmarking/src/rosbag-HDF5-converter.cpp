...
void RosbagHDF5Converter::iterateAndDumpPointclouds(const std::string& bagPath,
               const std::string& outputDir,
               const std::string& topicPointcloud,
               const int iterations)
{
    ...
    for (int i = 0; i<iterations; i++){
        ...
        {
            sensor_msgs::PointCloud2::ConstPtr message = m.instantiate<sensor_msgs::PointCloud2>();
...
void RosbagHDF5Converter::calcStats(std::vector<std::chrono::nanoseconds> durations){

    const size_t sz = durations.size();

    std::chrono::nanoseconds mean = std::accumulate(durations.begin(), durations.end(), std::chrono::nanoseconds(0))/sz;

    uint64_t var = 0;
    for( size_t n = 0; n < sz; n++ )
    {
        var += (durations.at(n).count() - mean.count()) * (durations.at(n).count() - mean.count());
    }
    var /= sz;
...
int main(int argc, char** argv)
{
    ...
    if(!std::filesystem::exists(bagPath)){
        std::cout << std::endl << std::endl << "The rosbag file does not exist!" << std::endl << std::endl;
        return 0;
    }

    if(!std::filesystem::exists(outputDir)){
        std::cout << std::endl << std::endl << "The output folder does not exist!" << std::endl << std::endl;
        return 0;
    }
